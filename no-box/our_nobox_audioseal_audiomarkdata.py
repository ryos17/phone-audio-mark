...
def load_and_resample(path: str, target_sr: int):
    """
    Load an audio file from `path` and resample it to `target_sr` (Hz).
    Returns:
      waveform: Tensor of shape (1, num_samples_at_target_sr)
      target_sr: the sample rate after resampling
    """
    # 1) Load from disk at its original sample rate
    waveform, orig_sr = torchaudio.load(path)

    # 2) If multi-channel, convert to mono by averaging channels
    if waveform.dim() > 1 and waveform.size(0) > 1:
        waveform = torch.mean(waveform, dim=0, keepdim=True)

    # 3) If the file’s SR ≠ target_sr, resample:
...
# Make sure Python can find the VISQOL package that Bazel built:
this_dir = os.path.dirname(__file__)

# project root is one up from “no-box”
project_root = os.path.abspath(os.path.join(this_dir, os.pardir))
visqol_pkg = os.path.join(project_root, "visqol", "bazel-bin", "python")

# add it to front of sys.path if not already there
if os.path.isdir(visqol_pkg) and visqol_pkg not in sys.path:
    sys.path.insert(0, visqol_pkg)
...
def parse_arguments():
    ...
    parser.add_argument("--resample_rate", "-sr", type=int, default=16000, help="Target sample rate (Hz) for all audio I/O")
    parser.add_argument("--model_prefix", type=str, default='', help="Enter prefix for model to evaluate")
    parser.add_argument("--mp3_bitrates", nargs='+', type=int, default=[8,16], help="List of MP3 bitrates (kbps) for the mp3 perturbation")

    return parser.parse_args()

def encode_audio_files(model, data_dir, output_dir, max_length, args):
    total_ba = []
    total_probs = []
    total_counts = 0
    
    file_list = [file for file in os.listdir(data_dir)[:args.testset_size] if file.endswith('.mp3')]
    print(f"Encoding {len(file_list)} files")
    print(file_list[:10])
    for file in tqdm(file_list, desc="Encoding Watermarks"):
        file_path = os.path.join(data_dir, file)
        waveform, sample_rate = load_and_resample(file_path, args.resample_rate)
        ...
        watermarked_signal = watermarked_signal.detach().cpu().squeeze(0)
        ...
        detector = AudioSeal.load_detector(f"{args.model_prefix}audioseal_detector_16bits").to(device=next(model.parameters()).device)
        detector.eval()
        # Compute FPR
        with torch.no_grad():
...
def decode_audio_files(model, output_dir, batch_size, args):
    total_ba = []
    total_probs = []
    total_counts = 0
    ...
    with open(output_file, 'w') as txt_file:
        ...
        for id, file in progress_bar:
            ...
            watermarked_signal, sr = load_and_resample(path, args.resample_rate)
...
def decode_audio_files_perturb(model, output_dir, common_perturbation, args):
    ...
    if common_perturbation == "time_stretch":
        ...
        for speed_factor in speed_factor_list:
            ...
            original_msgs = []
            path_list = []
            visqol = api_visqol(args.resample_rate)
            ...
            with open(output_file, 'w') as txt_file:
                ...
                for id, file in progress_bar:
                    ...
                    path = os.path.join(output_dir, file)
                    path_list.append(extract_id(path))
                    waveform, sample_rate = load_and_resample(path, args.resample_rate)
        ...
        for nq in n_q_values:
            ...
            with open(output_file, 'w') as txt_file:
                ...
                for id, file in progress_bar:
                    ...
                    waveform_pert, sample_rate = load_and_resample(os.path.join(output_dir_pert, waveform_pert_file), args.resample_rate)
        ...
        for bandwidth in bandwidth_values:
            ...
            with open(output_file, 'w') as txt_file:
                ...
                for id, file in progress_bar:
                    ...
                    path_list.append(extract_id(path))
                    waveform, sample_rate = load_and_resample(path, args.resample_rate)
                    waveform_pert, sample_rate = load_and_resample(os.path.join(encodec_cache, file), args.resample_rate)
                    snr = compute_snr(waveform.squeeze().unsqueeze(0),waveform_pert)
                    batch_SNRs.append(snr)
        ...
        bitrate_list = args.mp3_bitrates
        for bitrate in bitrate_list:
            total_ba = []
            total_probs = []
            total_SNRs = []
            ...
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')][: args.testset_size]
            print(f"Decoding {len(file_list)} files")
            print(file_list[:10])
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
            ...
            with open(output_file, 'w') as txt_file:
                ...
                for id, file in progress_bar:
                    ...
                    path = os.path.join(output_dir, file)
                    path_list.append(extract_id(path))
                    # resample & load at the chosen rate
                    waveform, sample_rate = load_and_resample(path, args.resample_rate)
                    if waveform.size(1) > args.max_length:
                        waveform = waveform[:, : args.max_length]
                    # pass along the sample_rate so pert_mp3 uses the right SR
                    waveform_pert = pert_mp3(waveform, bitrate, sample_rate)
                    if waveform_pert.size(1) > args.max_length:
                        waveform_pert = waveform_pert[:, : args.max_length]
...
def main():
    ...
    args.max_length = int(5*args.resample_rate)
    print(args)
    ...
    model = AudioSeal.load_generator(f"{args.model_prefix}audioseal_wm_16bits").to(device)
    ...
    if args.encode:
        encode_audio_files(model, data_dir, output_dir, args.max_length, args)
    if args.common_perturbation != '':
        decode_audio_files_perturb(model, dataset_dir, args.common_perturbation, args)
    else:
        decode_audio_files(model, dataset_dir, args.batch_size, args)
